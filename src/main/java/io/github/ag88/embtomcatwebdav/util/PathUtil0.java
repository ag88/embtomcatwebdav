...
public class PathUtil0 {

	Log log = LogFactory.getLog(PathUtil0.class);
    ...
    protected final void checkPath(String path) {
        if (path == null || path.length() == 0 || path.charAt(0) != '/') {
            throw new IllegalArgumentException("abstractResourceSet.checkPath".concat(path));
        }
    }
    ...
    public final File file(String name, boolean mustExist) {
        ...
        if (!canPath.equals(absPath)) {
            if (!canPath.equalsIgnoreCase(absPath)) {
                ...
                // that it was ignored.
                log.warn(String.format("symlink ignored %s, %s, %s", 
