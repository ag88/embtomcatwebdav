...
public class WebdavPath  {
    ...
    WebdavPath(String path) {
        this.elements = new ArrayList<String>();
        parsePathStr(path);
    }

    WebdavPath(ArrayList<String> elements, boolean absolute) {
	 ...
	 * @see WebdavPath#setCurrentWorkPath(WebdavPath path)
	 */
    public WebdavPath toAbsolutePath() {
    	if(isAbsolute())
    		return this;
    	...
    	if(currentWorkPath != null)     		
    		...
    		return new WebdavPath(DEFAULT_ROOT_PATH).resolve(this);
    }
    ...
    public WebdavPath getFileName() {
    	if(elements.size()==0) {
    		...
    		WebdavPath wp = new WebdavPath(elements.get(elements.size()-1));
    		if (isabsolute)
     ...
     */
    public WebdavPath getName(int index) {     		
    	if(elements.size() == 0 && index==0) {
    		...
    		WebdavPath wp = new WebdavPath(elements.get(index));
    		if(isabsolute) {
     ...
     */
    public Iterator<WebdavPath> iterator() {
    	ArrayList<WebdavPath> ret = new ArrayList<>(elements.size());
    	for(int i=0; i< elements.size(); i++) {
    		WebdavPath p = getName(i);
    		ret.add(p);
    	}
    	return ret.iterator();
    }
    ...
    public WebdavPath getParent() {
        ...
        return new WebdavPath(elms, true );
    }
    ...
    public WebdavPath getRoot() {
    	if (isabsolute)
    		return new WebdavPath(DEFAULT_ROOT_PATH);
    	else
    		return null;
    }
    ...
    public boolean startsWith(String other) {
        //throw new UnsupportedOperationException();
    	return startsWith(new WebdavPath(other));
    }
     ...
     * @return true, if successful
     */
    public boolean endsWith(WebdavPath other) {
        //throw new UnsupportedOperationException();
    	if(!(other instanceof WebdavPath))
    		return false;
    ...
    public boolean endsWith(String other) {
        //throw new UnsupportedOperationException();
    	return endsWith(new WebdavPath(other));
    }
     ...
     * @return the path
     */
    public WebdavPath normalize() {
        try {
            URI normal = new URI(getPathString()).normalize();
            return new WebdavPath(normal.getPath());
     ...
     */
    public WebdavPath relativize(Path other) {
    	if (other == null)
    	...
    	WebdavPath relpath = new WebdavPath( re, false);
    	//save the workpath
    	if (isabsolute) {    		
    		relpath.setCurrentWorkPath(new WebdavPath( workpath, true));
    	}    		    	
        return relpath;         
    }
	 ...
	 * @see WebdavPath#toAbsolutePath()
     */
    public WebdavPath resolve(WebdavPath other) {    	
        if (other == null) 
        	throw new NullPointerException();
        ...
        return new WebdavPath(ret, isAbsolute());
    }
     ...
     * @return the path
     */
    public WebdavPath resolve(String other) {
        return resolve(new WebdavPath(other));
    }
     ...
     */
    public WebdavPath resolveSibling(WebdavPath other) {
        //throw new UnsupportedOperationException();
    	if(!(other instanceof WebdavPath))
    		throw new IllegalArgumentException(NEED_TO_BE_AN_INSTANCE_OF_WEBDAV_PATH);
    	...
    	if(getParent()==null) {
    		return new WebdavPath(new ArrayList<String>(), false).resolve(other);    			
    	} else
     ...
     * @return the path
     */
    public WebdavPath resolveSibling(String other) {
        //throw new UnsupportedOperationException();
    	return resolveSibling(new WebdavPath( other));
    }
     ...
     */
    public WebdavPath subpath(int beginIndex, int endIndex) {
        //throw new UnsupportedOperationException();
    	if(elements.size() == 0)
    	...
    	return new WebdavPath( subp, false);
    ...
    public URI toUri(String scheme, String host, int port) {
        ...
        URI sardineUri;
        try {
            sardineUri = new URI(scheme, null, host, port, getPathString(), null, null);
            return sardineUri;
        } catch(URISyntaxException e) {
            throw new IOError(e);
        }
